...
if __name__ == '__main__':
    ...
    # trained_model = train_model(model, dataloader, num_epochs=10000, lr=1e-4)

    # Load the state dict with strict=False to ignore keys like pos_embedding.
    state_dict = torch.load("checkpoints/checkpoint_epoch_3100.pth", map_location="cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    generate_checkerboard_movie(dataset, trained_model, num_rows=2, num_cols=60, start_timestamp=START_TIME, threshold=THRESHOLD, output_filename="checkerboard_movie.mp4")
